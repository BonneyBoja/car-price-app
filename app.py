...
# 🚪 THE LOGIN PORTAL
# ==========================================
# This automatically creates the login UI and checks passwords!
authenticator.login()
...
# ==========================================
# 🆘 ACCOUNT HELP (Always show if not logged in)
# ==========================================
if not st.session_state["authentication_status"]:
    
    # 1. FORGOT PASSWORD EXPANDER
    with st.expander("❓ Forgot Password?"):
        try:
            username_forgot, email_forgot, new_password = authenticator.forgot_password()
            if username_forgot:
                st.success('Password successfully reset!')
                # Real world: You email this. University Project: We just print it to the screen!
                ...
                # Save the new temporary password to the yaml file
                with open('config.yaml', 'w') as file:
                    yaml.dump(config, file, default_flow_style=False)
        except Exception as e:
            st.error(e)

    # 2. REGISTRATION EXPANDER
...
# ==========================================
# 🚗 THE MAIN APP (If SUCCESSFULLY logged in)
# ==========================================
elif st.session_state["authentication_status"]:
    # 1. Sidebar Controls (Logout & Change Password)
    # 1. Sidebar Controls (Logout & Change Password)
    with st.sidebar:
        ...
        # 👇 WRAP THE RESET PASSWORD IN AN EXPANDER 👇
        with st.expander("⚙️ Account Settings / Reset Password"):
            try:
                if authenticator.reset_password(st.session_state["username"]):
                    st.success('Password modified successfully')
                    # Save the new password to the yaml file
                    with open('config.yaml', 'w') as file:
                        yaml.dump(config, file, default_flow_style=False)
            except Exception as e:
                st.error(e)
    ...
    # ==========================================
    # 👇 YOUR EXACT ORIGINAL APP CODE GOES BELOW 👇
    # ==========================================
    # (Just make sure it is indented one tab inward under this 'elif' block!)
